...
def reduce(line, target_len):
    ans_list = []
    while target_len > 0:
        len_line = len(line)
        idx = np.argmax(line[:len_line-target_len+1])
        ans_list.append(line[idx])
        line = line[idx+1:]
        target_len -= 1
        if len(line) == target_len:
            ans_list.extend(line)
            break
    return int("".join(map(str, ans_list)))
...
def solve(path):
    ...
    for line in input:
        ans += reduce(list(map(int, line.strip())), 12)

    return int(ans)

EXAMPLE_ANS = 3121910778619
